...
class DateUtils {
  ...
  static const String kDateWithYearFormat = 'd MMMM yyyy';
  ...
  static DateTime addMonthes(int nb) {
    DateTime now = DateTime.now();
    return now.addMonthes(nb);
  }
...
extension DateTimeExtensions on DateTime {
  ...
  String formatDate({String format}) =>
      DateFormat(format ?? 'd MMMM').format(this);

  String formatTime({String format}) =>
      DateFormat(format ?? 'HH:mm').format(this);

  DateTime addYear() => copyWith(year: year + 1);

  DateTime addMonthes(int nb) => copyWith(month: month + nb);
  ...
  bool isSameMonth(DateTime other) => other != null && month == other.month;
  ...
  String dateToString() => this.toIso8601String();
}
